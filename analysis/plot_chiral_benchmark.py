...
def calibration_points(ev, fs):
    ...
    for r in ev:
        ...
        fs_row = next(
            (x for x in fs
             if x["J"] == r["J"] and x["chi"] == r["chi"]
             and x["L"] == r["L"] and x["beta"] == r["beta"]
             and x["B"] == 0.0),
            None,
        )
        ...
        label = next(
            m[0] for m in MODELS
            if m[1] == r["J"] and m[2] == r["chi"]
        )
        points.append(
            dict(label=label, L=r["L"], beta=r["beta"],
                 gap_rel=gap_rel, t_score=t_score)
        )
    return points
...
def fig1_calibration(ev, fs, out_dir):
    ...
    ax.plot(x_ref, 40 * x_ref, color="gray", lw=0.8, ls="--",
            alpha=0.5, zorder=1, label="linear guide")

    ax.set_xlabel("T-score")
    ax.set_ylabel("relative gap $(F_{\\mathrm{mf}}-F_{\\mathrm{ex}})/|F_{\\mathrm{ex}}|$ (%)")
    ax.set_xlim(0, 1.65)
    ax.set_ylim(0, 72)
    ax.grid(True)
    ...
    for L in [2, 3, 4]:
        ax.scatter([], [], s=20 + 12 * (L - 2), color="gray",
                   label=f"$L={L}$", edgecolors="white", linewidths=0.4)
...
def fig2_tscore_by_model(ev, fs, out_dir):
    ...
    for bi, beta in enumerate(BETAS):
        ...
        bars = ax.bar(x + (bi - 1) * width, means, width,
                      color=BETA_COLORS[beta], label=f"$\\beta={beta}$",
                      edgecolor="white", linewidth=0.4)

    ax.axhline(1.0, color="gray", lw=0.8, ls="--", alpha=0.7,
               label="T-score = 1")
    ax.set_xticks(x)
    ax.set_xticklabels([m[4].replace("Heis.", "Heis.") for m in MODELS],
...
def fig3_varconv(nf, out_dir):
    ...
    for ax, L, beta, title in panels:
        for name, J, chi, color, marker, full_label in MODELS:
            ...
            if not nfs:
                continue
            ax.plot(nfs, vrat, color=color, marker=marker,
                    markersize=4, label=full_label,
                    linestyle="--" if name == "pure chiral" else "-")
        ax.axhline(1.0, color="gray", lw=0.7, ls=":", alpha=0.6)
        ax.set_xlabel("numfields")
        ax.set_ylabel("variance ratio $\\mathrm{Var}[\\hat{F}](n_f)/\\mathrm{Var}[\\hat{F}](0)$")
        ax.set_title(title)
        ax.grid(True)
...
def fig5_gap_vs_B(fs, out_dir):
    ...
    for name, J, chi, color, marker, full_label in MODELS:
        rows = sorted_by(
            [r for r in select(fs, J=J, chi=chi, L=L, beta=beta)
             if r["f_exact"] is not None],
            "B",
        )
        if not rows:
        ...
        ax.plot(Bs, gaps, color=color, marker=marker, markersize=3,
                label=full_label)
    ...
    ax.legend(fontsize=7, framealpha=0.9)
    ax.set_title(
        f"Relative free-energy gap vs $B$\n($L={L}$, $\\beta={beta}$)"
    )
...
def fig6_kappa_z(fs, out_dir):
    ...
    for ax, (name, J, chi, color, marker, full_label) in zip(axes, chiral_models):
        for L, ls in [(6, "-"), (8, "--")]:
            ...
            ax.plot(Bs, kzs, color=color, ls=ls, lw=1.2,
                    label=f"$L={L}$")
    ...
    ax = axes[1]
    ax.axvspan(0, 0.4, alpha=0.08, color="red",
               label="frozen min. ($L=8$)")
    ax.legend(fontsize=7, framealpha=0.9)
